...
export default function InventoryHeatmapPage({ siteInventory, stateInventory, addresses }: { siteInventory: SiteInventorySummary[]; stateInventory: Record<string, number>; addresses: WarehouseAddress[] }) {
  ...
  const make = (key: typeof internationalRegions[number], label: string, x: number, y: number): WorldMapPoint => {
    ...
    return { key, label, x, y, value: quantity / inventoryHeatMax, display: `${quantity.toLocaleString('zh-CN')} 件`, ratioDisplay: internationalTotal > 0 ? `${((quantity / internationalTotal) * 100).toFixed(1)}%` : '0%' }
  }
  ...
  return <div className="page">
    ...
    <section className="section map-section">
      ...
      <div className="map-layout">
        <div className="us-map"><UsStatesMap stateValues={stateInventory} warehouses={warehouses} valueLabel="在库量" maxValue={inventoryHeatMax}/></div>
    ...
    <section className="section map-section">
      <div className="section-heading"><div><h2>加拿大、英国和欧洲库存分布</h2><p>真实国家轮廓按在库量与在途量合计上色，并与美国各州共用同一库存色阶；海外仓库不进入全美库存占比。</p></div></div>
      <div className="map-layout">

...
function WarehouseMap({ addresses, results, history }: { addresses: WarehouseAddress[]; results: AnalysisResult[]; history: HistoricalSummary }) {
  ...
  return <section className="section map-section"><div className="section-heading"><div><h2><MapPinned size={20}/>美国仓库与需求分布</h2><p>州级热力表示美国历史订单数量，不等同于仓库库存数量。</p></div><StatusTag tone={history.postcodeCoverage >= .8 ? 'success' : 'warning'}>有效邮编覆盖率 {(history.postcodeCoverage * 100).toFixed(1)}%</StatusTag></div>
    <div className="map-layout"><div className="us-map"><UsStatesMap stateValues={history.stateDemand} warehouses={warehouses} valueLabel="订单量"/></div>

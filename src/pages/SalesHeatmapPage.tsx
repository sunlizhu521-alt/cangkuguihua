    ...
    <section className="section map-section">
      ...
      <div className="map-layout heatmap-wide-layout">
        ...
        <div className="map-side">
          {(['美西', '美中', '美东'] as WarehouseRegion[]).map((region) => <div className="region-stat" key={region}><span className={`region-dot ${region}`}/><div><strong>{region}</strong><small>{channelText(region)}</small></div><div className="region-values"><b>{((history.regionDemand[region] ?? 0) * 100).toFixed(1)}%</b><small>合计历史发货</small></div></div>)}
          {internationalRegions.map((region) => <div className="region-stat" key={region}><span className="region-dot"/><div><strong>{region}</strong><small>{channelText(region)}</small></div><div className="region-values"><b>{((history.regionDemand[region] ?? 0) * 100).toFixed(1)}%</b><small>{internationalDemand[region] > 0 ? `合计 ${internationalDemand[region].toLocaleString('zh-CN')} 件` : '无订单数据'}</small></div></div>)}

...
import { demandRegion, demandSiteRegion, inspectWorkbook, localQuoteDraft, parseForecast, parseInventory, parseOutbound, parsePackaging, parseWarehouses, postalRegion, readMappedRows } from './fileParser'
...
import type { AiSettings, AnalysisResult, AnalysisSettings, DemandRegion, FileSlotDefinition, ManualTransferQuote, PageId, QuoteVersion, SiteInventorySummary, SiteRegion, StoredFile, WarehouseAddress } from './types'
import './styles.css'
...
function buildHistory(files: StoredFile[]): HistoricalSummary {
  ...
  if (!amazonFile || !merchantFile) return { channelAmazonShare: 0, channelMerchantShare: 0, postcodeCoverage: 0, regionDemand: { 美西: 0, 美中: 0, 美东: 0, 英国: 0, 欧洲: 0 }, siteDailyDemand: { 美国: 0, 加拿大: 0, 英国: 0, 欧洲: 0 }, messages: ['亚马逊仓配与商家自发货历史出库数据未同时通过校验，不生成正式地区建议'] }
  ...
  if (!start || !end || start > end) return { channelAmazonShare: 0, channelMerchantShare: 0, postcodeCoverage: 0, regionDemand: { 美西: 0, 美中: 0, 美东: 0, 英国: 0, 欧洲: 0 }, siteDailyDemand: { 美国: 0, 加拿大: 0, 英国: 0, 欧洲: 0 }, messages: ['两类历史出库没有共同覆盖日期区间，不生成正式地区建议'] }
  ...
  all.forEach((row) => { const region = demandSiteRegion(row.postalCode); if (region) siteAmount[region] += row.quantity })
...
function emptyHistoricalSummary(): HistoricalSummary {
  return { channelAmazonShare: 0, channelMerchantShare: 0, postcodeCoverage: 0, regionDemand: { 美西: 0, 美中: 0, 美东: 0, 英国: 0, 欧洲: 0 }, siteDailyDemand: { 美国: 0, 加拿大: 0, 英国: 0, 欧洲: 0 }, messages: ['请完成历史出库数据映射并运行分析'] }
}

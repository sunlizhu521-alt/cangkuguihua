...
export function exportAnalysisWorkbook(
  ...
  const sources = files.map((file) => ({ 文件槽位: fileSlots.find((slot) => slot.id === file.slotId)?.label ?? file.slotId, 文件名: file.fileName, 更新时间: file.updatedAt, 数据行数: file.rowCount, 映射状态: file.validation, 已选择字段: Object.keys(file.mapping).join('、') }))
